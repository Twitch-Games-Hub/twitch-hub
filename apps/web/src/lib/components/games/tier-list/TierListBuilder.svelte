...
<div class="w-full max-w-4xl mx-auto p-6 bg-gray-900 rounded-lg border border-gray-800">
  ...
  <div class="space-y-4 mb-8">
    {#each tiers as tier}
      <div class="flex items-stretch gap-4 h-24 rounded-lg overflow-hidden">
        <!-- Tier Label -->
        <button
          class="px-6 py-4 font-bold text-lg rounded-l-lg {tierLabelColors[tier] ||
            'bg-gray-700 text-white'} cursor-default flex-shrink-0 flex items-center justify-center w-16"
          disabled={true}
        >
          {tier}
        </button>
        ...
        <!-- Tier Content Area -->
        <div
          class="flex-1 border-l-4 {tierColors[tier] ||
            'border-l-gray-500 bg-gray-500/10'} rounded-r-lg p-4 cursor-pointer transition-colors hover:bg-opacity-20 flex flex-wrap items-center gap-3 content-start overflow-y-auto"
  ...
  <div class="mb-6">
    ...
    <div
      class="bg-gray-800/50 rounded-lg p-4 border border-gray-700 min-h-24 flex flex-wrap items-center gap-3 content-start overflow-y-auto"
    >
      ...
      {:else}
        {#each getUnplacedItems() as item (item)}
          <button
            class="px-4 py-2 rounded-full font-medium text-sm transition-all flex-shrink-0 {selectedItem ===
            item
    ...
    {#if selectedItem}
      <p class="text-sm text-gray-400 mt-3">
        Selected: <span class="text-blue-400 font-semibold">{selectedItem}</span> - Click a tier to place
        it
      </p>
    {/if}
  </div>
