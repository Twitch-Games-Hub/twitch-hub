<script lang="ts">
  ...
  let { success = false, canceled = false }: { success?: boolean; canceled?: boolean } = $props();
...
{:else}
  ...
  {#if !subscriptionStore.isSubscriber}
    <div class="mb-6">
      <h2 class="text-sm font-semibold text-text-primary mb-3">Subscribe for Unlimited Sessions</h2>
