...
<svelte:head>
  <title>Profile - Twitch Hub</title>
</svelte:head>
          ...
          <div class="space-y-3">
            ...
            <div class="flex flex-wrap gap-4 text-sm text-text-muted">
              {#if profile.stream.game_name}
                <span
                  >Playing <span class="text-text-secondary">{profile.stream.game_name}</span></span
                >
              {/if}
                  ...
                  >
                    <div class="flex items-center gap-2">
                      {#if liveStreamIds.has(channel.broadcaster_id)}
                        <span class="h-2 w-2 flex-shrink-0 rounded-full bg-success-500" title="Live"
                        ></span>
                      {:else}
